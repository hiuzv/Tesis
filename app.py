...
def get_user_history(user_ip):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT role, message FROM chat_history_ip WHERE user_id = %s AND "timestamp"-CURRENT_TIMESTAMP < ''03:00:00.0000000'' ORDER BY timestamp DESC LIMIT 6', (user_ip,))
